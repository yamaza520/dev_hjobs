                            ...
                            <?php if (count($result)): ?>
                                <?php foreach ($result as $row): ?>
                                    <tr role="row">
                                        ...
                                        <td><?php echo $row->last_name_kana . " " . $row->first_name_kana;?></td>
                                        <td><?php echo $row->gender == "m" ? "男性" : "女性" ;?></td>
                                        <td>
                                            <?php if (!empty($row->birth_year)): ?>
                                                <?php echo $row->birth_year . "年 " . $row->birth_month . "月 " . $row->birth_day . "日";?>
                                            <?php endif ?>
                                        </td>
                                        <td><?php echo $row->email;?></td>
                                        <td><?php echo $row->phone;?></td>
                                        <td>〒<?php echo $row->zip_code . " " . $row->pref_name . " " . $row->address;?></td>
                                        <td><?php echo $row->is_working == 1 ? "離職中" : "就業中" ;?></td>
                                        <td><?php echo $row->marital_status == 1 ? "既婚" : "未婚" ;?></td>
                                        <td class="col-sm-2">
                                            <?php echo anchor($base_path . 'job_seeker/setup/'.$row->id, '変更', array('class' => 'btn bg-orange btn-flat btn-sm')); ?>
...
<div class="modal fade" id="confirm-modal">
    <div class="modal-dialog">
        <div class="modal-content">
            <?php echo form_open($base_path . 'job_seeker/delete', array('method' => 'post'));?>
