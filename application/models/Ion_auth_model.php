...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $group_id = null)
	{
		...
		$fields = $this->identity_column . ', email, users.id, first_name, last_name, password, phone, active, last_login, profile_status';
		$fields .= ', group_id, '.$this->tables['groups'].'.description as group_desc';
        ...
        if ($group_id) {
            if (is_array($group_id)) {
                $this->db->where_in($this->tables['users_groups'].'.group_id', $group_id);
            } else {
                $this->db->where($this->tables['users_groups'].'.group_id', $group_id);
            }
        }
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				if ($user->group_id == GROUP_JOBSEEKER) {
					...
					$user->child = $this->Job_seeker_model->find_one_by(array('user_id' =>$user->id));
				}
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'group_desc'           => !empty($user->group_desc) ? $user->group_desc : '',
			'profile_status'       => $user->profile_status,
			'child'                => isset($user->child) ? $user->child : null,
		);
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, email, first_name, last_name, phone, last_login, profile_status')
